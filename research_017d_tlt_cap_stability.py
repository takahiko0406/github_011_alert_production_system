...
OUT_PREFIX = "model_c_plus_017D_tlt_cap_stability"
EXTRA_ASSETS = sorted(set(r16a.EXTRA_ASSETS + ["TLT", "IEF", "SHY"]))
EXEC_ASSETS_EXT = sorted(set(r16a.EXEC_ASSETS_EXT + ["TLT"]))
...
def build_daily_bond_features(prices):
    d = pd.DataFrame(index=prices.index)
    for window in [3, 5, 10, 21]:
        for asset in ["TLT", "IEF", "SHY", "BIL", "QQQM", "SOXX", "IWM"]:
        ...
        d[f"tlt_vs_bil_{window}"] = d[f"tlt_ret_{window}"] - d[f"bil_ret_{window}"]
        d[f"tlt_vs_risk_{window}"] = d[f"tlt_ret_{window}"] - d[f"risk_avg_ret_{window}"]
        d[f"ief_vs_shy_{window}"] = d[f"ief_ret_{window}"] - d[f"shy_ret_{window}"]
    return d.fillna(0.0)


def daily_tlt_signal(day_features, macro_row, params):
    ...
    tlt_ret_5d = float(day_features.get("tlt_ret_5", 0.0))
    tlt_ret_10d = float(day_features.get("tlt_ret_10", 0.0))
    tlt_vs_bil_5d = float(day_features.get("tlt_vs_bil_5", 0.0))
    tlt_vs_risk_5d = float(day_features.get("tlt_vs_risk_5", 0.0))
    tlt_vs_risk_10d = float(day_features.get("tlt_vs_risk_10", 0.0))
    ...
    equity_stress = (
        ...
        or tlt_vs_risk_5d >= params["tlt_vs_risk_5_min"]
    )
    score = (
        0.25 * tlt_vs_bil_5d
        + 0.25 * tlt_vs_risk_5d
        + 0.15 * tlt_vs_risk_10d
        + 0.15 * rate_easing_10d
        + 0.10 * max(0.0, risk_off)
        + 0.10 * max(0.0, -growth_state)
    )
    on = (
        equity_stress
        and tlt_ret_5d >= params["tlt_5_min"]
        and tlt_ret_10d >= params["tlt_10_min"]
        and tlt_vs_bil_5d >= params["tlt_vs_bil_5_min"]
        and rate_easing_10d >= params["rate_easing_10_min"]
...
def apply_tlt_overlay(weights, day_features, macro_row, params):
    weights = dict(weights)
    on, score = daily_tlt_signal(day_features, macro_row, params)
    if not on:
    ...
    if source_total <= 0:
        return weights, False, score
    move = min(source_total * params["source_take"], params["tlt_cap"])
    if move <= 1e-12:
        return weights, False, score
    ...
    for asset in source_assets:
        ...
        if weight > 0:
            ...
            weights["TLT"] = weights.get("TLT", 0.0) + cut
...
def run_backtest(rebalance, prices, cfg, params, tx_cost=m.DEFAULT_TRANSACTION_COST):
    ...
    bond_features = build_daily_bond_features(prices)
    ...
    for i, date in enumerate(rebal_dates):
        ...
        for j, hold_date in enumerate(hold_dates):
            ...
            day_weights, tlt_on, tlt_score = apply_tlt_overlay(day_weights, bond_features.loc[hold_date], row, params)
            for asset in EXEC_ASSETS_EXT:
        ...
        log = {
            ...
            "daily_tlt_last_score": last_tlt_score,
...
def main():
    ...
    logs_by_name = {}
    returns_by_name = {}
    for cap, source_take, tlt10, tlt_vs_risk, rate10, source_group in itertools.product(
        [0.08, 0.10, 0.12, 0.15],
        [0.25, 0.35, 0.50],
        [-0.005, 0.000],
        [0.005, 0.010, 0.020],
        ...
        params = {
            ...
            "tlt_vs_risk_5_min": tlt_vs_risk,
            "rate_easing_10_min": rate10,
            "risk_min": 0.00,
            "risk_max": 1.75,
            "crash_max": 2.0,
            "breakdown_min": 1.0,
        ...
        name = (
            f"017D_cap{cap:.2f}_take{source_take:.2f}_tlt10{tlt10:.3f}_"
            f"edge{tlt_vs_risk:.3f}_rate{rate10:.3f}_{source_group}"
        )
        ...
        row.update({
            ...
            "tlt_rebalance_count": int((log["daily_tlt_active_days"] > 0).sum()),
    ...
    cols = [
        ...
        "tlt_rebalance_count", "tlt_active_days", "tlt_last_avg_weight", "bil_last_avg_weight",
        "source_group", "tlt_cap", "source_take", "tlt_10_min",
        "tlt_vs_risk_5_min", "rate_easing_10_min",
    ]
