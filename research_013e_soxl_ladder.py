...
OUT_PREFIX = "model_c_plus_013E_soxl_ladder"
...
def ladder_soxl_fraction(row, params):
    ...
    if risk > params["soxl_risk_max"] or ma63 < params["ma63_min"]:
        return 0.0
    if leadership >= params["large_lead"] and rel_mom >= params["large_mom"]:
        return params["large_frac"]
    if leadership >= params["medium_lead"] and rel_mom >= params["medium_mom"]:
        return params["medium_frac"]
    if leadership >= params["small_lead"] and rel_mom >= params["small_mom"]:
        return params["small_frac"]
    return 0.0
...
def apply_overlay(weights, row, params):
    ...
    soxx_on = (
        ...
        and crash <= 1.25
        and breakdown <= 2.0
        and leadership >= params["soxx_lead_min"]
    )
    if not soxx_on:
    ...
    soxl_frac = ladder_soxl_fraction(row, params)
    soxl_on = False
    if soxl_frac > 0:
...
def main():
    ...
    for qfrac, cap, small, medium, large, small_mom, med_mom, large_mom in itertools.product(
        [0.75, 0.90],
        [0.25, 0.30],
        [0.25, 0.35],
        [0.50, 0.65],
        ...
        name = (
            f"013E_q{qfrac:.2f}_cap{cap:.2f}_"
            f"f{small:.2f}-{medium:.2f}-{large:.2f}_"
            f"mom{small_mom:.2f}-{med_mom:.2f}-{large_mom:.2f}"
        )
        ...
        row.update({
            ...
            "beats_early_return": early_metrics["ann"] > base_early["ann"],
            "beats_early_sharpe": early_metrics["sharpe"] > base_early["sharpe"],
            "beats_recent_return": recent_metrics["ann"] > base_recent["ann"],
            "beats_recent_sharpe": recent_metrics["sharpe"] > base_recent["sharpe"],
            "balanced_score": (
                (common_metrics["ann"] - base_common["ann"]) * 1.5
                + (common_metrics["sharpe"] - base_common["sharpe"]) * 0.45
                + (early_metrics["sharpe"] - base_early["sharpe"]) * 0.35
    ...
    cols = [
        ...
        "balanced_score", "soxx_overlay_rebalances", "soxl_overlay_rebalances",
